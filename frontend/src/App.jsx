...
import {
  ...
  ChevronRight, XCircle, AlertCircle, Loader2, Wifi, WifiOff,
  TrendingDown, Server, Database, ArrowRight, Eye, List,
  ChevronDown, ChevronUp, Hash, Cpu, Globe
} from 'lucide-react'
...
function StatusBadge({ status, pulse = false }) {
  ...
  return (
    <span className={clsx('status-badge border', cfg.bg, cfg.color)}>
      {pulse && <span className={clsx('w-1.5 h-1.5 rounded-full animate-pulse', cfg.dot)} />}
      <Icon size={12} />
      {cfg.label}
    </span>
  )
  ...
  return (
    <div className="flex items-center gap-1 w-full overflow-x-auto scrollbar-none py-2">
      {PIPELINE_STAGES.map((stage, idx) => {
        ...
        const errored = isError && stage.step === currentStep

        return (
          <React.Fragment key={stage.key}>
            <div className={clsx(
              'flex flex-col items-center gap-1.5 min-w-[80px] transition-all duration-500',
              done    ? 'opacity-100' : '',
              active  ? 'opacity-100' : '',
              ...
              <div className={clsx(
                ...
                active && !errored ? 'bg-brand-500/20 border-brand-500 text-brand-400 animate-pulse' : '',
            ...
            {idx < PIPELINE_STAGES.length - 1 && (
              <div className={clsx(
                'flex-1 h-px min-w-[8px] transition-all duration-700',
                done ? 'bg-emerald-500/50' : 'bg-surface-400',
              )} />
            )}
          </React.Fragment>
        )
...
function LogsTab({ incident }) {
  ...
  const lineColor = (line) => {
    ...
    if (upper.includes('[CRITICAL]')) return 'text-red-400'
    if (upper.includes('[ERROR]'))    return 'text-orange-400'
  ...
  return (
    <div className="code-block max-h-96 overflow-y-auto animate-fade-in">
      ...
      {lines.length === 0 ? (
        ...
        lines.map((line, i) => (
          <div key={i} className="flex gap-3 leading-6">
            ...
            <span className={clsx('break-all', lineColor(line))}>{line}</span>
          </div>
        ))
      )}
    </div>
    ...
    <div className="space-y-3 animate-fade-in">
      {agents.map(agent => {
        ...
        return (
          <div
            key={agent.key}
            className={clsx('glass-card border overflow-hidden transition-all duration-300', agent.borderColor)}
          >
            <button
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className={clsx('font-mono text-sm font-semibold', agent.color)}>
                  {agent.label}
                </span>
                {data ? (
            ...
            {isOpen && (
              <div className="border-t border-white/5 p-4 space-y-4">
                {!data ? (
                  <p className="text-slate-500 text-sm">Waiting for {agent.label} to complete...</p>
                ) : (
                  <>
                    {/* Reasoning chain */}
                        ...
                        <div className="space-y-1.5">
                          {data.reasoning_chain.map((step, i) => (
                            <div key={i} className="reasoning-item">
                              <span className="text-brand-400 font-mono text-xs shrink-0 pt-0.5">
                    ...
                    {agent.key === 'AUDITOR' && data.root_cause && (
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          <div className="p-2.5 rounded-lg bg-surface-700">
                            <span className="text-slate-400">Category</span>
                            <p className="text-slate-100 font-mono mt-0.5">{data.error_category || '—'}</p>
                          </div>
                          <div className="p-2.5 rounded-lg bg-surface-700">
                            <span className="text-slate-400">Blast Radius</span>
                            <p className="text-slate-100 font-mono mt-0.5">{data.blast_radius || '—'}</p>
                          </div>
                          <div className="p-2.5 rounded-lg bg-surface-700">
                          ...
                          <div className="p-2.5 rounded-lg bg-surface-700">
                            ...
                            <p className="text-red-400 font-mono mt-0.5">{data.recommended_severity || '—'}</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {agent.key === 'VALIDATOR' && (
                      <div className={clsx(
                        ...
                        <div>
                          <p className={clsx(
                            'font-bold text-sm',
                            data.verdict === 'EXECUTE' ? 'text-emerald-400' : 'text-red-400',
                          )}>
...
function ScriptTab({ incident, agentTrail }) {
  ...
  return (
    <div className="animate-fade-in space-y-4">
      ...
      {commands.length > 0 && (
        <div>
          <h4 className="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-3">
            Approved Commands ({commands.length})
          </h4>
          <div className="space-y-2">
            {commands.map((cmd, i) => (
              <div key={i} className="glass-card p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={clsx(
                      ...
                      cmd.risk_level === 'MEDIUM' ? 'bg-yellow-500/10 text-yellow-400'  :
                                                    'bg-red-500/10 text-red-400',
                    )}>
                      {cmd.risk_level || 'LOW'} risk
                    </span>
                  </div>
                ...
                <div className="code-block py-2 px-3">
                  ...
                  <span className="text-slate-200 break-all">{cmd.command}</span>
                </div>
                {cmd.rollback_command && (
                  <div className="code-block py-2 px-3 opacity-50">
                    <span className="text-yellow-400"># rollback: </span>
                    <span className="text-slate-400 break-all">{cmd.rollback_command}</span>
                  </div>
                )}
              </div>
            ))}
      ...
      {script ? (
        <div>
          ...
          <pre className="code-block max-h-64 overflow-y-auto text-xs text-slate-300 leading-6">
            {script}
          </pre>
        </div>
      ) : (
        <div className="glass-card p-8 text-center">
          ...
          <p className="text-slate-400 text-sm">
            {['TRIGGERED', 'AUDITOR_DIAGNOSING'].includes(incident?.status)
              ? 'Waiting for Patcher to generate remediation commands...'
  ...
  const metrics = [
    {
      ...
      value: formatINR(financial.total),
      sub: 'Estimated loss if unresolved',
      icon: AlertTriangle,
      color: 'text-red-400',
    ...
    },
    {
      ...
      value: formatINR(financial.revenue_loss || impact.revenueProtectedINR),
      sub: 'Downtime revenue impact averted',
      icon: TrendingDown,
      color: 'text-emerald-400',
    ...
    },
    {
      ...
      value: formatINR(financial.sla_penalty || impact.slaPenaltyAvoidedINR),
      sub: 'P1 breach penalty prevented',
      icon: Shield,
      color: 'text-blue-400',
      iconBg: 'bg-blue-500/10',
    },
    {
      label: 'Engineering Cost Saved',
      value: formatINR(financial.engineering_cost),
      sub: `~${impact.engineeringHoursSaved || 2} eng-hours @ ₹3,500/hr`,
      icon: Cpu,
      color: 'text-violet-400',
    ...
    },
    {
      ...
      value: `${impact.downtimePreventedMinutes || auditorResult?.estimated_downtime_minutes || 0} min`,
      sub: 'Customer-facing downtime averted',
      icon: Clock,
      color: 'text-amber-400',
      iconBg: 'bg-amber-500/10',
    },
    {
      label: 'Auto-Healed By',
      value: resolution?.resolvedBy || '—',
      sub: resolution?.resolvedAt ? `at ${new Date(resolution.resolvedAt).toLocaleTimeString()}` : 'Pending',
      icon: Zap,
  ...
  if (!auditorResult) {
    return (
      <div className="glass-card p-8 text-center animate-fade-in">
        ...
        <p className="text-slate-400 text-sm">
          Cost analysis will be available after AUDITOR completes diagnosis.
        </p>
      </div>
    )
  }
      ...
      {resolution && (
        <div className="p-4 rounded-xl border border-emerald-500/30 bg-gradient-to-r from-emerald-950/40 to-teal-950/40">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-emerald-400/70 uppercase tracking-wide font-semibold">
                Total Savings from Auto-Remediation
              </p>
              <p className="text-2xl font-bold text-emerald-400 font-mono">
                {formatINR(impact.costSavedINR || financial.total)}
              </p>
            </div>
          </div>
        </div>
        ...
        {metrics.map((metric, i) => {
          ...
          return (
            <div key={i} className="metric-card">
              ...
              <p className={clsx('text-xl font-bold font-mono', metric.color)}>
                {metric.value}
              </p>
      ...
      {resolution?.executionLog && resolution.executionLog.length > 0 && (
        <div>
          ...
          <div className="flex gap-3">
            <div className="metric-card flex-1 text-center">
              <p className="text-2xl font-bold text-emerald-400 font-mono">{resolution.commandsSucceeded}</p>
              <p className="text-xs text-slate-400">Commands OK</p>
            </div>
            <div className="metric-card flex-1 text-center">
              <p className="text-2xl font-bold text-red-400 font-mono">{resolution.commandsFailed}</p>
              <p className="text-xs text-slate-400">Commands Failed</p>
            </div>
            <div className="metric-card flex-1 text-center">
              <p className="text-2xl font-bold text-brand-400 font-mono">
                {(resolution.totalDurationMs / 1000).toFixed(1)}s
              </p>
...
function IncidentCard({ incident, isSelected, onClick }) {
  ...
  const Icon = cfg.icon
  const isTerminal = ['AUTO_REMEDIATED', 'ESCALATED_TO_SRE', 'ESCALATED', 'PIPELINE_ERROR'].includes(incident.status)
  ...
  return (
    <button
      ...
      className={clsx(
        'w-full text-left p-4 rounded-xl border transition-all duration-200 hover:border-brand-500/40',
        isSelected
          ? 'border-brand-500/50 bg-brand-950/20'
          : 'border-white/5 bg-surface-700/30 hover:bg-surface-700/60',
      )}
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex items-center gap-2 min-w-0">
          <div className={clsx(
            ...
            cfg.bg.includes('emerald') ? 'bg-emerald-500/10' :
            cfg.bg.includes('red')     ? 'bg-red-500/10'     :
            cfg.bg.includes('violet')  ? 'bg-violet-500/10'  :
                                         'bg-brand-500/10',
          )}>
        ...
        {incident.severity && (
          <span className={clsx(
            'text-[10px] px-1.5 py-0.5 rounded border font-bold shrink-0',
            SEVERITY_COLORS[incident.severity] || SEVERITY_COLORS.P3,
          )}>
            {incident.severity}
          </span>
        )}
      ...
      <p className="text-xs text-slate-300 mb-2 truncate font-medium">
        {incident.service || 'Unknown Service'}
      </p>

      <div className="flex items-center justify-between">
        <StatusBadge status={incident.status} pulse={isActive} />
...
export default function App() {
  ...
  const fetchIncidents = useCallback(async () => {
    try {
      ...
      // Auto-refresh selected incident if it's still active
      if (selectedIdRef.current) {
        const updated = (data.incidents || []).find(i => i.incidentId === selectedIdRef.current)
        if (updated) {
  ...
  // Fetch a single incident + agent trail
  ...
  const handleSelectIncident = useCallback(async (incident) => {
    setSelectedIncident(incident)
    setActiveTab('logs')
    await fetchIncidentDetail(incident.incidentId)
  }, [fetchIncidentDetail])
  ...
  const handleTrigger = async () => {
    ...
    try {
      ...
      // Immediately fetch to show the new incident
      await fetchIncidents()

      // Auto-select the new incident
      if (data.incidentId) {
        const newInc = { incidentId: data.incidentId, status: 'TRIGGERED', ...data }
        setSelectedIncident(newInc)
        setActiveTab('logs')
        setTimeout(() => fetchIncidentDetail(data.incidentId), 1000)
      }
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      await fetchIncidents()
      if (selectedIdRef.current) {
        await fetchIncidentDetail(selectedIdRef.current)
      }
    }, POLL_INTERVAL_MS)
  ...
  const activeIncidents = incidents.filter(i =>
    !['AUTO_REMEDIATED', 'ESCALATED_TO_SRE', 'ESCALATED', 'PIPELINE_ERROR'].includes(i.status)
  )
  const resolvedIncidents = incidents.filter(i =>
    ['AUTO_REMEDIATED', 'ESCALATED_TO_SRE', 'ESCALATED', 'PIPELINE_ERROR'].includes(i.status)
  )

  const isTerminalStatus = selectedIncident &&
    ['AUTO_REMEDIATED', 'ESCALATED_TO_SRE', 'ESCALATED', 'PIPELINE_ERROR'].includes(selectedIncident.status)
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div>
              ...
              <span className="hidden sm:inline text-xs text-slate-500 ml-2">
                Autonomous Cloud Incident Triage
              </span>
            </div>
          </div>
          ...
          <div className="hidden md:flex items-center gap-5 text-xs">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-slate-500">
                {lastRefresh ? `${timeAgo(lastRefresh.toISOString())}` : 'Never'}
              </span>
            </div>
          </div>
          ...
          <aside className="w-72 shrink-0 hidden lg:flex flex-col gap-4">
            ...
            <div className="glass-card p-4">
              ...
              {loading ? (
                <div className="space-y-2">
                  {[1, 2].map(i => (
                    <div key={i} className="h-20 rounded-xl bg-surface-700/40 animate-pulse" />
                  ))}
                </div>
            ...
            {resolvedIncidents.length > 0 && (
              <div className="glass-card p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs bg-surface-500 text-slate-400 px-1.5 py-0.5 rounded-full font-bold">
            ...
            {!selectedIncident && !loading && (
              <div className="glass-card p-16 text-center animate-fade-in">
                ...
                <p className="text-slate-400 text-sm max-w-md mx-auto mb-6">
                  Autonomous cloud incident triage powered by Amazon Bedrock.
            ...
            {selectedIncident && (
              <>
                  ...
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {selectedIncident.severity && (
                          <span className={clsx(
                            ...
                            SEVERITY_COLORS[selectedIncident.severity] || SEVERITY_COLORS.P3,
                          )}>
                            {selectedIncident.severity}
                          </span>
                        )}
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">
                        {selectedIncident.errorType} — {selectedIncident.description || selectedIncident.region}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <StatusBadge
                        ...
                        pulse={!isTerminalStatus}
                      />
                      {!isTerminalStatus && (
                        <button
                          onClick={() => fetchIncidentDetail(selectedIncident.incidentId)}
                ...
                <div className="glass-card p-5 animate-fade-in">
                  ...
                  {/* Tab content */}
                  {activeTab === 'logs' && (
                    <LogsTab incident={selectedIncident} />
                  )}
                  {activeTab === 'reasoning' && (
                    <ReasoningTab incident={selectedIncident} agentTrail={agentTrail} />
                  )}
                  {activeTab === 'script' && (
                    <ScriptTab incident={selectedIncident} agentTrail={agentTrail} />
                  )}
                  {activeTab === 'impact' && (
                    <ImpactTab incident={selectedIncident} agentTrail={agentTrail} />
                  )}
                </div>
              </>
            )}
      ...
      <footer className="border-t border-white/5 mt-8 py-4">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 flex items-center justify-between text-xs text-slate-500">
          <div className="flex items-center gap-4">
            ...
            <span className="hidden sm:inline">Powered by Amazon Bedrock + Step Functions</span>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5">
